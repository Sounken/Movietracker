...
| **J14** | Carrousel « Cinémas » : *Avengers : Endgame* proposé alors qu'il n'est pas en salle en France | ✅ `/movie/now_playing` abandonné — même avec `region=FR` il remontait ressorties et titres jamais distribués ici (Endgame, date renvoyée 2019-04-24, **4ᵉ position** le 21/09), et n'offre ni réglage de fenêtre ni filtre de type de sortie. Remplacé par `/discover/movie` avec `region=FR` + `release_date` (la fenêtre porte sur la sortie **française**), `with_release_type=3` (sortie nationale ; le type 2, sortie limitée, est écarté car il fait entrer les projections de festival) et une fenêtre de **30 jours**, alignée sur la pill Streaming. Mesuré : 71 films au vivier, aucune ressortie |
