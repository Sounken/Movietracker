...
import CompareClient from "../../../compare/[id]/CompareClient";
import styles from "../../../compare/[id]/compare.module.css";
...
export default async function SeriesComparePage({
  params,
}: {
  ...
  if (theirId === session.userId) redirect("/series/profile");
  ...
  const result = await compareSeries(session.userId, theirId);
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        ...
        <h1 className={styles.headerTitle}>Vos séries face à celles de {theirName}</h1>
      </div>
      ...
      <div className={styles.stats}>
        ...
        <div className={styles.stat}>
          <div className={styles.statLab}>Séries en commun</div>
          <div className={styles.statVal}>{result.common.length}</div>
          <div className={styles.statSub}>notées par vous deux</div>
        </div>
        ...
        <div className={styles.stat}>
          ...
          <div className={styles.statSub}>sur chaque série commune</div>
        </div>
      ...
      <CompareClient common={result.common} mediaBase="/series" />
    </div>
  );
}
